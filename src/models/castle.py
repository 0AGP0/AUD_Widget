...
class Castle:
    ...
    width: float
    height: float
    inventory: Dict[str, int] = field(default_factory=dict)  # Envanter
    
    def __post_init__(self):
        """Başlangıç değerlerini ayarla"""
        # Kalenin çarpışma alanı - kalenin alt kısmı
        self.rect = QRectF(self.x, self.y - self.height, self.width, self.height)
        print(f"Kale oluşturuldu: ({self.x}, {self.y}), {self.width}x{self.height}")
        
        # Başlangıç envanterini ayarla
        self.inventory = {
            "odun": 0,
            "erzak": 0,
            "altın": 0
    ...
    def contains_point(self, x: float, y: float) -> bool:
        """Verilen nokta kalenin içinde mi kontrol et"""
        return self.rect.contains(x, y)
    
    def get_entrance(self) -> tuple[float, float]:
        """Kalenin giriş noktasını döndür"""
        # Varsayılan olarak kalenin alt orta noktası
